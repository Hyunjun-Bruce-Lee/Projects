...
base_path = '.ETRI/'
base_path = 'C:/ALL/4_bert_download_004_bert_eojeol_tensorflow/004_bert_eojeol_tensorflow'
weight_path = 'C:/ALL/CODES/Python_scripts/ETRI_KOBERT/CANDIDATE_MODEL'
con_dict = {
    'config_path' : os.path.join(base_path, 'bert_config.json'),
    'checkpoint_path' : os.path.join(base_path, 'model.ckpt'),
    'vocab_path' : os.path.join(base_path, 'vocab.korean.rawtext.list'),
    'T_weight_path' : os.path.join(weight_path, 'model(siam)_data(klue)_batch(20)_epoch(1)_loss(mse)_ADAM(0.00001).h5'),
    'C_weight_path' : os.path.join(weight_path, 'sentiment_bert_naver_ai_batch10_seq302_ALL.h5'),
}
